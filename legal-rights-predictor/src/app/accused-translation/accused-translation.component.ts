...
import { ACCUSED_PREDICT_MARATHI_ENDPOINT, ACCUSED_TRANSLATE_MARATHI_ENDPOINT, API_URL, PREDICT_MARATHI_ENDPOINT, SAVE_CHAT_ENDPOINT, TRANSLATE_MARATHI_ENDPOINT } from 'src/app/constants';

@Component({
  selector: 'app-accused-translation',
  templateUrl: './accused-translation.component.html',
  styleUrls: ['./accused-translation.component.css']
})
export class AccusedTranslationComponent implements OnInit {
  ...
  sendMessage() {
      ...
      this.http.post(`${API_URL}${ACCUSED_PREDICT_MARATHI_ENDPOINT}`, { query: this.userInput })
      .subscribe((res: any) => {
        ...
        if (answers.length > 1) {
         ...
         this.http.post(`${API_URL}${ACCUSED_TRANSLATE_MARATHI_ENDPOINT}`, { query: formattedAnswers })
         .subscribe((res: any) => {
